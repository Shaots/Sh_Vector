...
public:
    ...
    /*
     * Logging is supposed to be implemented by receiving RC error code and writing corresponding
     * string to output
     *
     * It means that you have to store pre-defined {RC: string} pairs in any way
     *
     * It's recommended to use std::map<RC, std::string> to call std::map::operator[] for easy
     * string retrieval
     *
     ...
     *
     */
    virtual RC log(RC code, Level level, const char* const& srcfile, const char* const& function,
                   int line) = 0;
     ...
     */
    virtual RC severe(RC code) { return log(code, Level::SEVERE); };
     ...
     */
    virtual RC warning(RC code) { return log(code, Level::WARNING); };
     ...
     */
    virtual RC info(RC code) { return log(code, Level::INFO); };
